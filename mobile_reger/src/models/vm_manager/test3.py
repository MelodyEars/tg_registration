...
def vm_manager(name_avd, proxy=None):
	custom_config = f' -no-snapshot-save -memory {memory} -netdelay none -netspeed full -port {port} -accel auto -timezone {timezone} -no-boot-anim -screen multi-touch'
	if proxy:
		custom_config = custom_config + f" -http-proxy {proxy}"
